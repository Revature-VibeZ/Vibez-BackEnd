...
-- create 6 "posts" or 1 post with 4 comments of varying depth, and a second top level post
    ...
    VALUES
    (1, 1, 'top level post! parent id is null. click on me to see comments.', CURRENT_TIMESTAMP, 'imageurl', null, 'first title');

    -- second is a "comment" which is just a nested post with parent id of 1, referencing the first post we just created.
    INSERT INTO public.posts
    (id, author_id, content, timestamp, image, parent_id, title)
    VALUES
    (2, 2, 'comment or nested post, parent id is 1', CURRENT_TIMESTAMP, 'imageurl', 1, 'second title');
    ...
    (id, author_id, content, timestamp, image, parent_id, title)
    VALUES
    (3, 3, 'reply or 2+ levels deep post to comment 1, parent id is 2', CURRENT_TIMESTAMP, 'imageurl', 2, 'third title');
    ...
    (id, author_id, content, timestamp, image, parent_id, title)
    VALUES
    (6, 3, 'what is your favorite cat food??', CURRENT_TIMESTAMP, 'imageurl', null, 'another title');
